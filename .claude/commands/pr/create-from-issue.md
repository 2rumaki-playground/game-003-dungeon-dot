...
5. **仕様書同期の確認**: 変更が `docs/spec/` 配下の仕様ドキュメントに影響するか確認する
   - `constants.py` を変更した場合 → `docs/spec/constants.md` の更新が必要か確認
   - 新しい仕様ドキュメントファイルを追加した場合 → `docs/spec/readme.md` の一覧に追記し、仕様書全体の目次が最新になっていることを確認
   - その他のソースコード変更 → 関連する `docs/spec/` のドキュメントに影響がないか確認
   - 仕様ドキュメントの更新が必要な場合は、対応するドキュメントも更新してコミットに含める
6. **コミット前チェック**（各コミットの前に必ず実行）:
   - `task format`
   - `task lint`
   - `task typecheck`
   - `task deps:check`
   - `task test`
7. **コミット**: Conventional Commits形式、日本語、50文字以内
...
1. 変更したファイルの中に以下が含まれる場合、対応する仕様ドキュメントの更新が必要か確認する:
   - `constants.py` → `docs/spec/constants.md` の更新が必要か確認
   - `domain/`, `presentation/`, `infrastructure/` 配下のファイル → 関連する `docs/spec/` のドキュメントに影響がないか確認
2. 仕様ドキュメントの更新が必要な場合は、対応するドキュメントも更新してコミットに含める
3. 新しい仕様ドキュメントを追加した場合は `docs/spec/readme.md` の一覧も更新する
